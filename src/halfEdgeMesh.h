 ...
 * first?) Here we just say that one iterator comes before another if the
 * address of the object it points to is smaller.  (You should not have to worry
 * about this!)
 */
...
public:
  ...
  HalfedgeCIter next(void) const {
    return _next;
  } ///< access the next half edge (const iterator)
...
public:
   ...
   * otherwise
   */
  bool isBoundary(void) const { return _isBoundary; }
...
public:
  ...
  Vector3D newPosition; ///< For Loop subdivision, this will be the updated
                        ///< position of the vertex
  bool isNew; ///< For Loop subdivision, this flag should be true if and only if
              ///< this vertex is a new vertex created by subdivision (i.e., if
              ///< it corresponds to a vertex of the original mesh)
  ...
  Vector3D centroid; ///< average of neighbor positions, storing the value
                     ///< computed by Vertex::computeCentroid()
...
protected:
  HalfedgeIter
      _halfedge; ///< one of the halfedges "rooted" or "based" at this vertex
};
...
public:
  ...
  Vector3D newPosition; ///< For Loop subdivision, this will be the position for
                        ///< the edge midpoint
  bool isNew; ///< For Loop subdivision, this flag should be true if and only if
              ///< this edge is a new edge created by subdivision (i.e., if it
              ///< cuts across a triangle in the original mesh)

  EdgeRecord record;

protected:
  HalfedgeIter
      _halfedge; ///< one of the two halfedges associated with this edge
};
...
public:
   ...
   * vertex indices. The input must describe a manifold, oriented surface, where
   * the orientation of a polygon is determined by the order of vertices in the
   * list.
   */
   ...
   * when a mesh is passed as a constant reference, we would instead write
   *
   *    for( VertexCIter v = ... )
   *
   ...
   * iterator) to the new element. (These methods cannot have const versions,
   * because they modify the mesh!)
   */
   ...
   * carefully about exactly when and how to delete mesh elements, since other
   * elements will often still point to the element that is being deleted, and
   * accessing a deleted element will cause your program to crash (or worse!).
   * A good exercise to think about is: suppose you're iterating over a linked
   * list, and want to delete some of the elements as you go.  How do you do
   * this without causing any problems?  For instance, if you delete the current
   ...
   */
  EdgeIter flipEdge(
      EdgeIter e); ///< flip an edge, returning a pointer to the flipped edge
  VertexIter splitEdge(
      EdgeIter e); ///< split an edge, returning a pointer to the inserted
                   ///< midpoint vertex; the halfedge of this vertex should
                   ///< refer to one of the edges in the original mesh
